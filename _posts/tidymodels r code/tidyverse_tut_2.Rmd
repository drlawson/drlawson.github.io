...
Trying the model without the recipe now just to see if the recipe improved it at all 

```{r}
flight_data_2 <- 
  flights %>%
  mutate(
    #convert the arrival delay to a factor (aka categorical instead of the number of minutes late)
    arr_delay = ifelse(arr_delay >= 30, "late", "on_time"),
    #labeling those 30 or more minutes late as late and all others on time
    arr_delay = factor(arr_delay),
    #makes it into a factor with 2 levels 
    date = lubridate::as_date(time_hour)
    #code that makes looking at dates easier, takes the time/hour/date data from the original data set and changes it to only the date
  )%>%
    #include weather data - where is this from and where does it go???
  inner_join(weather, by =  c("origin", "time_hour")) %>%
  #only retain the specific columns we will use (excludes several columns)
...
Shows that the results are much less accurate without using the recipe 